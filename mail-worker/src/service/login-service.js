...
const loginService = {
	...
	async loginWithKey(c, params) {
		const { key } = params;
		const loginKey = c.env.LOGIN_KEY;
		const loginUserEmail = c.env.LOGIN_USER_EMAIL;

		if (!loginKey || !loginUserEmail) {
			throw new BizError(t('loginKeyNotConfigured'), 501);
		}
